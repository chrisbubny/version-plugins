 ...
 * Description: Custom workflow for Test Method post types with approval process and version control
 * Version: 1.2.0
...
public function restrict_publish_button() {
        ...
        if ($post_type !== 'test_method' || empty($post)) return;
        ...
        if ($is_admin) {
            ...
            jQuery(document).ready(function($) {
                ...
                $('#publish').click(function(e) {
                    ...
                    // For new posts or revisions, always prompt for version type (no basic option)
                    e.preventDefault();
                    
                    var versionOption = prompt("Is this a minor or major version change?", "minor");
                    
                    if (versionOption === 'minor' || versionOption === 'major') {
                        if ($('#version_update_type').length) {
                            $('#version_update_type').val(versionOption);
                        } else {
                            $('form#post').append('<input type="hidden" name="version_update_type" value="' + versionOption + '">');
                        }
                    ...
                    // Allow basic updates to published content
                    e.preventDefault();
                    ...
                    const unsubscribe = wp.data.subscribe(function() {
                        ...
                        if (publishButton && !publishButton.getAttribute('data-custom-handler')) {
                            ...
                            publishButton.addEventListener('click', function(e) {
                                <?php if ($post_status != 'publish' || $is_revision): ?>
                                // For new posts or revisions, always prompt for version type
                                e.preventDefault();
                                e.stopPropagation();
                                ...
                                if (versionOption === 'minor' || versionOption === 'major') {
                                    ...
                                    formData.append('version_type', versionOption);
...
public function set_version_before_publish() {
     try {
         ...
         // CRITICAL FIX: For 'none' version type (basic updates), DO NOT change version
    ...
    public function admin_enqueue_scripts($hook) {
        global $post_type;
        if ($post_type != 'test_method') return;
...
wp_localize_script('test-method-workflow', 'testMethodWorkflow', array(
            ...
            'strings' => array(
                'confirm_submit' => __('Are you sure you want to submit this test method for review?', 'test-method-workflow'),
                'confirm_approve' => __('Are you sure you want to approve this test method?', 'test-method-workflow'),
                'confirm_reject' => __('Are you sure you want to reject this test method?', 'test-method-workflow'),
                'confirm_unlock' => __('Are you sure you want to unlock this test method for editing? This will move it back to draft status.', 'test-method-workflow'),
                'confirm_publish_revision' => __('Are you sure you want to publish this approved revision? It will replace the original test method.', 'test-method-workflow')
            )
        ));
    }
