	  ...
	  public function unlock_test_method() {
		  try {
			  ...
			  wp_send_json_success(array(
				  'message' => __('Test method unlocked successfully', 'test-method-workflow'),
				  'reload' => true
			  ));
		  } catch (Exception $e) {
	...
	public function add_unlock_button_to_editor() {
		...
		jQuery(document).ready(function($) {
			...
			$('#tmw-unlock-button').on('click', function(e) {
				...
				if (confirm('<?php echo esc_js(__('Are you sure you want to unlock this test method? This will move it back to draft status and allow editing.', 'test-method-workflow')); ?>')) {
					$.ajax({
						url: ajaxurl,
						type: 'POST',
						data: {
